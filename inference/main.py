...
class Inference:
    def __init__(self, ckpt, transforms, estimator, scale_translation=1):
        ...
        dataset_test = DroneCubeDataset(
            ...
            split="train",
            zip_file_name=self.config.train.dataset_zip_file_name_training,
        )

        self.test_loader = torch.utils.data.DataLoader(
            dataset_test,
            batch_size=self.config.train.batch_training_size,
            num_workers=0,
            drop_last=True,
        )
        self.model.eval()
...
def test_model():
    config = get_config("Model/config-local.yml")
    ...
    i = Inference(
        "inference/drone_target_pose_estimation_ep4.tar",
