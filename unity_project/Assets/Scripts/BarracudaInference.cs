...
public class BarracudaInference : MonoBehaviour
{
    ...
    public IEnumerator StartPoseEstimationInternal()
    {
        ...
        RenderTexture rt2 = RenderTexture.GetTemporary(rt.width, rt.height,0,rt.graphicsFormat);
        ...
        if (Application.platform == RuntimePlatform.WebGLPlayer)
            RenderTexture.active = rt;//mainCamera.targetTexture;
        else
        ...
        Tensor tensor;
        if (Application.platform == RuntimePlatform.WebGLPlayer)
            tensor = new Tensor(rt, 3, "test");
        else
            tensor = new Tensor(rt2, 3, "test");
