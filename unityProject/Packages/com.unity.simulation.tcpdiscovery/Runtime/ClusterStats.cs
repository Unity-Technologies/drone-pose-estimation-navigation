    ...
    public partial class ClusterManager : IDisposable
    {
        ...
        public void LogStats()
        {
            lock (_peerStatistics)
            {
                ...
                {
                    Log.I(string.Format("RTT for address {0} min {1} max {2} avg {3}", kv.Key,
                        kv.Value.minRTT.ToString(printFloatPrecision),
                        kv.Value.maxRTT.ToString(printFloatPrecision),
                        kv.Value.avgRTT.ToString(printFloatPrecision)));
                }
            }
            lock (_messageStatistics)
            {
                ...
                {
                    Log.I(string.Format("RTT for message {0} min {1} max {2} avg {3}", Utils.FourCCToString(kv.Key),
                        kv.Value.minRTT.ToString(ClusterManager.printFloatPrecision),
                        kv.Value.maxRTT.ToString(ClusterManager.printFloatPrecision),
                        kv.Value.avgRTT.ToString(ClusterManager.printFloatPrecision)));
                }
            }
        }
