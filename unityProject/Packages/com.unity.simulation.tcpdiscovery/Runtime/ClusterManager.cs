...
namespace Unity.Simulation.UdpCluster
{
    ...
    public partial class ClusterManager : IDisposable
    {
        ...
        )
        {
            ...
            Log.I($"[ClusterManager] Initialize Cluster listening on port {options.port}");
            ...
            RegisterHandler(kMessageIdIdentify, true, (Message message) =>
            {
                ...
                {
                    Log.V($"[ClusterManager] Cluster disovery completed in {ElapsedTime()} second(s).");
            ...
            SendMessage(this.MulticastClusterNode, kMessageIdIdentify, options.identifyClusterTimeoutSec, (OutboundMessage message) =>
            {
                if (message.timedOut)
                {
                    Log.V($"[ClusterManager] Cluster disovery timed out after {options.identifyClusterTimeoutSec} second(s).");
        ...
        void _ReceiveThreadFunc()
        {
                    ...
                    else
                    {
                        if (receiveTask.IsCompleted)
                        {
                            do
                            {
                                ...
                                string messageFourCc = Utils.FourCCToString(receiveData.messageId);
                                string instanceId = Utils.InstanceToString(receiveData.instanceId);
                                string payloadPreview = Utils.HexDump(datagram.data, datagram.length);
                                Log.V($"[ClusterManager] _ReceiveThreadFunc: Received messageId \"{messageFourCc}\" from instanceId \"{instanceId}\" with data: {payloadPreview}");
                        ...
                        {
                            Log.W("[ClusterManager] _ReceiveThreadFunc: UDP receive task canceled");
                        }

                        else if (receiveTask.IsFaulted)
                        {
                            Log.E($"[ClusterManager] _ReceiveThreadFunc: UDP receive task encountered an exception: {receiveTask.Exception}");
                        }
            ...
            catch (ObjectDisposedException e)
            {
                ...
                if (_run)
                {
                    Log.V($"[ClusterManager] _ReceiveThreadFunc: Object Disposed Exception: {e.Message}");
                }
            }
            catch (SocketException e)
            {
                ...
                if (_run)
                {
                    Log.V($"[ClusterManager] _ReceiveThreadFunc: Socket Exception: {e.Message}");
                }
            }
        }
            ...
            while (_run)
            {
                ...
                for (var i = 0; i < inboundCount; ++i)
                {
                    ...
                    if (message.arrivalTime + message.timeoutSecs < time)
                    {
                        ...
                        string messageFourCc = Utils.FourCCToString(message.messageId);
                        string instanceId = Utils.InstanceToString(message.instanceId);
                        Log.V($"[ClusterManager] Expiring inbound message \"{messageFourCc}\" from instanceId \"{instanceId}\" after {message.timeoutSecs} second(s).");
                    }
                }
            }
        }
        ...
        {
            string messageFourCc = Utils.FourCCToString(receiveData.messageId);
            string instanceIdName = Utils.InstanceToString(receiveData.instanceId);

            if (receiveData.recipientId != 0 && receiveData.recipientId != UniqueIdentifier)
            {
                Log.V($"[ClusterManager] _VerifyReceivedMessage: Ignoring received data intended for recipient using same address. messageId \"{messageFourCc}\", instanceId \"{instanceIdName}\", uniqueId \"{receiveData.recipientId.ToString("X7")}\"");
                return false;
            }

            if (receiveData.secret != Options.secret)
            {
                Log.V($"[ClusterManager] _VerifyReceivedMessage: Rejecting message from foreign cluster address {receiveData.address}");
                return false;
            }
            ...
            {
                Log.V($"[ClusterManager] _VerifyReceivedMessage: Ignoring received data from self. messageId \"{messageFourCc}\", instanceId \"{instanceIdName}\"");
                return false;
            }
            ...
            lock (_mutex)
            {
                ...
                if (receiveData.messageId == kMessageIdAck)
                {
                    ...
                    if (_outboundMessages.TryGetValue(ack.instanceId, out outbound))
                    {
                        ...
                        string messageFourCc = Utils.FourCCToString(outbound.messageId);
                        Log.V($"[ClusterManager] _ReceiveMessage: Ack \"{messageFourCc}\" took {(elapsedTime - outbound.lastTime).ToString(printFloatPrecision)} second(s). Dispatch delay {ack.avgMessageDispatchTime.ToString(printFloatPrecision)} second(s)");
                ...
                else
                {
                    ...
                    {
                        Log.V($"[ClusterManager] _ReceiveMessage: Dispatching messageId \"{messageFourCc}\" from instanceId \"{instanceIdName}\"");
                    ...
                    {
                        Log.V($"[ClusterManager] _ReceiveMessage: Already dispatched message \"{messageFourCc} from instanceId \"{instanceIdName}\"");
                    }
                    _AckMessage(message);
                }
            }
        ...
        int _AddNodeIfNotPresent(IPAddress address, uint nodeUniqueId)
        {
            if (nodeUniqueId == UniqueIdentifier)
            {
                Log.E($"[ClusterManager] _AddNodeIfNotPresent: Adding self as node: {nodeUniqueId}; this should never happen");
            ...
            lock (_mutex)
            {
                if (!_nodes.ContainsKey(nodeUniqueId))
                {
                    ...
                    Log.V($"[ClusterManager] _AddNodeIfNotPresent: Adding new node id {node.uniqueId.ToString("X8")} address {node.address}. Node count is now {NodeCount}");
                }
            }
        ...
        int _RemoveNodeIfPresent(uint nodeUniqueId)
        {
            lock (_mutex)
            {
                if (_nodes.ContainsKey(nodeUniqueId))
                {
                    ...
                    NodeCount = _nodes.Count;
                    NodeCountChanged?.Invoke(node, NodeCount, lastNodeCount);
                    Log.V($"[ClusterManager] _RemoveNodeIfPresent: Removing node id {node.uniqueId.ToString("X8")} address {node.address}. Node count is now {NodeCount}");
                }
            }
        ...
        void _StopSendingMessage(OutboundMessage message)
        {
            ...
            string messageFourCc = Utils.FourCCToString(message.messageId);
            string instanceIdName = Utils.InstanceToString(message.instanceId);
            Log.V($"[ClusterManager] _StopSendingMessage: Message \"{messageFourCc}\" from instanceId \"{instanceIdName}\" has completed. timeout {message.timedOut}");
        ...
        {
            string messageFourCc = Utils.FourCCToString(message.messageId, context);
            string instanceIdName = Utils.InstanceToString(message.instanceId);
            string payloadPreview = Utils.HexDump(message.payload, message.payload.Length);
            Log.V($"[ClusterManager] _Send: message \"{messageFourCc}\" from instanceId \"{instanceIdName}\" to {message.address}:{port} payload: {payloadPreview}");
        ...
        {
            Debug.Assert(Utils.IsMainThread(), "[ClusterManager] _DispatchMainThreadActions must be called on main thread.");
